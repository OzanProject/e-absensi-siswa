...
@section('title', 'Edit Akun Orang Tua: ' . $parent->name)
...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
    ...
    <h1 class="text-2xl font-bold text-gray-800 flex items-center mb-2 sm:mb-0">
        ...
        <span>Edit Akun Orang Tua: {{ $parent->name }}</span>
    </h1>
...
@section('content')
    <div class="bg-white rounded-xl shadow-lg border border-gray-100">
        <div class="p-6"> {{-- Padding disesuaikan --}}
            ...
            {{-- Notifikasi Error Sesi (Styling Tailwind) --}}
            @if(session('error')) 
            ...
            <form action="{{ route('parents.update', $parent->id) }}" method="POST" id="parentForm">
                @csrf
                @method('PUT')
                
                {{-- Validasi Error Global (Styling Tailwind) --}}
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <div class="space-y-6">
                            ...
                            @php
                                // Helper Class untuk Input Styling (Fokus Amber/Orange)
                            ...
                            <div>
                                ...
                                <input type="text" name="relation_status" id="relation_status" 
                                        ...
                                        value="{{ old('relation_status', $parent->relation_status) }}"
                            ...
                            <div>
                                ...
                                <div class="flex rounded-lg shadow-sm">
                                    ...
                                    {{-- Mengganti focus:ring-blue-500 ke focus:ring-amber-500 --}}
                                    <input type="text" name="phone_number" id="phone_number" 
                                            class="flex-1 block w-full rounded-none rounded-r-lg px-3 py-2 border border-gray-300 
                                                     text-sm focus:ring-amber-500 focus:border-amber-500 
                            ...
                            <div>
                                ...
                                <small class="mt-1 text-xs text-gray-500">Email harus unik (kecuali email yang sama).</small>
                            </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-6">
                                ...
                                <select name="student_ids[]" id="student_ids" 
                                    ...
                                    @foreach($students as $student) 
                                        @php
                                            ...
                                            $isTakenByOtherParent = ($student->parents->count() > 0) && !$student->parents->contains($parent->id);
                                            $isSelected = in_array($student->id, old('student_ids', $selectedStudentIds));
                                        @endphp

                                        <option value="{{ $student->id }}" 
                                                {{ $isSelected ? 'selected' : '' }}
                                                {{ $isTakenByOtherParent ? 'disabled' : '' }}
                                                title="{{ $isTakenByOtherParent ? 'Sudah Diampu Orang Tua Lain' : '' }}">
                                            {{ $student->name }} ({{ $student->class->name ?? 'Kelas N/A' }})
                                            @if($isTakenByOtherParent) (Sudah Diampu Orang Tua Lain) @endif
                                        </option>
                                    @endforeach
                                </select>
                                <small class="mt-1 text-xs text-gray-500 block">Hanya siswa yang belum memiliki Orang Tua, atau siswa yang sudah terhubung dengan akun ini, yang dapat dipilih.</small>
                                @error('student_ids') 
                            ...
                            {{-- Info & Tips --}}
                            {{-- Mengganti bg-gray-50 ke bg-orange-50 dan border/text --}}
                            <div class="bg-orange-50 border-l-4 border-orange-500 p-4 rounded-lg mt-4">
                                <h6 class="font-bold text-lg text-orange-700 flex items-center mb-2"><i class="fas fa-info-circle mr-2"></i> Penautan Siswa:</h6>
                                <ul class="list-disc ml-5 text-sm text-gray-700 space-y-1">
                                    <li>Siswa yang sudah terhubung ke akun ini akan otomatis terpilih.</li>
                                    <li>Anda dapat menambahkan/menghapus siswa yang **belum terhubung** ke orang tua lain.</li>
