...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
    @foreach($days as $day)
        <div
            ...
            {{-- Header Hari --}}
            <div
                class="bg-gradient-to-r from-gray-50 to-white px-6 py-4 border-b border-gray-100 flex justify-between items-center group">
                ...
                <span
                    class="text-xs font-bold {{ isset($schedules[$day]) ? 'text-indigo-600 bg-indigo-50' : 'text-gray-400 bg-gray-100' }} px-3 py-1 rounded-full border {{ isset($schedules[$day]) ? 'border-indigo-100' : 'border-gray-200' }}">
            ...
            <div class="p-6 flex-1 bg-white">
                                    ...
                                    <div
                                        ...
                                        <div class="flex justify-between items-start mb-2">
                                            <div>
                                                ...
                                                <div
                                                    class="flex items-center text-xs text-gray-500 mt-1 font-mono bg-gray-50 inline-block px-1.5 py-0.5 rounded">
                                            ...
                                            <div
                                                class="flex items-center space-x-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                                ...
                                                <form action="{{ route('admin.schedules.destroy', $schedule->id) }}" method="POST"
                                                    onsubmit="return confirm('Hapus jadwal mapel {{ $schedule->subject->name }} ini?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                        ...
                                        <div class="flex items-center pt-2 border-t border-gray-50 mt-2">
                                            <div
                                                class="w-5 h-5 rounded-full bg-gray-100 flex items-center justify-center text-gray-400 text-[10px] mr-2">
                ...
                @else
                    <div class="flex flex-col items-center justify-center h-40 text-gray-400">
                        ...
                        <span class="text-sm font-medium text-gray-500">Tidak ada jadwal</span>
                        <span class="text-xs text-gray-400 mt-1">Hari ini libur / kosong</span>
                    </div>
                @endif
            </div>
            ...
            {{-- Footer Card (Decor) --}}
            <div
                class="h-1.5 bg-gradient-to-r from-indigo-500 to-purple-500 w-0 group-hover:w-full transition-all duration-500">
            </div>
        </div>
    @endforeach
</div>
