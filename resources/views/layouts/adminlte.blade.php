...
<body class="font-inter antialiased bg-gray-100 text-gray-800 h-full">
    ...
    <div class="flex h-full">
        ...
        <div id="sidebar-overlay"
            class="fixed inset-0 bg-gray-900/50 z-30 hidden opacity-0 transition-opacity duration-300 md:hidden backdrop-blur-sm">
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (sidebar && toggleBtn) {
                const contentWrapper = document.querySelector('.flex-1.overflow-y-auto'); // Target wrapper

                toggleBtn.addEventListener('click', function (e) {
                    e.stopPropagation();

                    // Cek apakah di Mobile (< 768px) atau Desktop
                    if (window.innerWidth < 768) {
                        // LOGIKA MOBILE: Toggle Translate
                        ...
                        // LOGIKA DESKTOP: Toggle Margin & Translate
                        sidebar.classList.toggle('-translate-x-full'); // Hide sidebar to left

                        if (contentWrapper) {
                            // Jika sidebar hidden, hilangkan margin kiri wrapper
                            if (sidebar.classList.contains('-translate-x-full')) {
                                contentWrapper.classList.remove('md:ml-64');
                                contentWrapper.classList.add('md:ml-0');
                                // Adjust Header position too if it's fixed
                                ...
                                contentWrapper.classList.remove('md:ml-0');
                                // Restore Header
                                const header = document.querySelector('header.fixed');
                                if (header) {
