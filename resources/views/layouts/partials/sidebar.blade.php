...
@php
    ...
    $isManajemenDataActive =
        ...
        Request::is('admin/school-teachers*') ||
...
<aside id="main-sidebar"
    ...
    <div class="overflow-y-auto h-[calc(100vh-62px)] p-4" x-data="{ 
        activeDropdown: '{{ $isManajemenDataActive ? 'manajemen-data' : ($isAbsensiWaliKelasActive ? 'walikelas-absensi' : (Request::is('walikelas/report*') ? 'walikelas-report' : '')) }}' 
    }">
        <nav>
            <ul class="space-y-2">
                ...
                @if ($user)
                    ...
                    @if ($user->isSuperAdmin())
                            ...
                            <li>
                                <button
                                    @click="activeDropdown = activeDropdown === 'manajemen-data' ? null : 'manajemen-data'"
                                    ...
                                    <i class="fas fa-angle-left right ml-auto transition-transform duration-200"
                                       :class="activeDropdown === 'manajemen-data' ? '-rotate-90' : 'rotate-0'"></i>
                                </button>

                                <ul x-show="activeDropdown === 'manajemen-data'"
                                    x-collapse
                                    class="pl-2 pt-1 space-y-1"
                                    >
                                    @foreach ([
                                            'classes.index' => ['route' => 'classes.index', 'text' => 'Data Kelas'],
                                            'students.index' => ['route' => 'students.index', 'text' => 'Data Siswa'],
                                            'admin.school-teachers.index' => ['route' => 'admin.school-teachers.index', 'text' => 'Data Guru'],
                    ...
                    @elseif($user->isWaliKelas())
                        ...
                        <li>
                            <button
                                @click="activeDropdown = activeDropdown === 'walikelas-absensi' ? null : 'walikelas-absensi'"
                                ...
                                <i class="fas fa-angle-left right ml-auto transition-transform duration-200"
                                   :class="activeDropdown === 'walikelas-absensi' ? '-rotate-90' : 'rotate-0'"></i>
                            </button>
                            <ul x-show="activeDropdown === 'walikelas-absensi'"
                                x-collapse
                                class="pl-2 pt-1 space-y-1">
                                <li>
                                    <a href="{{ route('walikelas.absensi.scan') }}"
                                        class="flex items-center p-2 rounded-md transition duration-150 text-sm {{ isActive('walikelas/absensi/scan') ? $activeSubClass : $defaultSubClass }}">
                                        <i class="fas fa-qrcode text-xs w-3 h-3 mr-3 opacity-75"></i>
                                        <span class="ml-1">Scan Masuk/Pulang</span>
                                    </a>
                                </li>
                                <li>
                                    <a href="{{ route('walikelas.absensi.manual.index') }}"
                                        class="flex items-center p-2 rounded-md transition duration-150 text-sm {{ isActive('walikelas/absensi/manual*') ? $activeSubClass : $defaultSubClass }}">
                                        <i class="fas fa-edit text-xs w-3 h-3 mr-3 opacity-75"></i>
                                        <span class="ml-1">Manual & Koreksi</span>
                                    </a>
                                </li>
                            </ul>
                        </li>
                        ...
                        <li>
                            <button
                                @click="activeDropdown = activeDropdown === 'walikelas-report' ? null : 'walikelas-report'"
                                ...
                                <i class="fas fa-angle-left right ml-auto transition-transform duration-200"
                                   :class="activeDropdown === 'walikelas-report' ? '-rotate-90' : 'rotate-0'"></i>
                            </button>

                            <ul x-show="activeDropdown === 'walikelas-report'"
                                x-collapse
                                class="pl-2 pt-1 space-y-1">
                                <li>
                                    <a href="{{ route('walikelas.report.index') }}"
                                        class="flex items-center p-2 rounded-md transition duration-150 text-sm {{ isActive('walikelas/report') && !isActive('walikelas/report/monthly-recap') ? $activeSubClass : $defaultSubClass }}">
                                        <i class="fas fa-list-alt text-xs w-3 h-3 mr-3 opacity-75"></i>
                                        <span class="ml-1">Laporan Harian</span>
                                    </a>
                                </li>
                                <li>
                                    <a href="{{ route('walikelas.report.monthly_recap') }}"
                                        class="flex items-center p-2 rounded-md transition duration-150 text-sm {{ isActive('walikelas/report/monthly-recap') ? $activeSubClass : $defaultSubClass }}">
                                        <i class="fas fa-calendar-alt text-xs w-3 h-3 mr-3 opacity-75"></i>
                                        <span class="ml-1">Rekap Absensi Bulanan</span>
                                    </a>
                                </li>
                            </ul>
                        </li>
                    ...
                    @elseif($user->isGuru())
                        <li class="px-2 pt-3 pb-1 text-xs font-semibold text-indigo-400 uppercase border-t border-gray-700 mt-2 flex justify-between items-center">
                            <span>AREA GURU</span>
                            <i class="fas fa-chalkboard-teacher text-indigo-500/70"></i>
                        </li>
                        ...
                        <li>
                            <a href="{{ route('teacher.journals.index') }}"
                               class="flex items-center p-2.5 rounded-md transition duration-150 {{ isActive('guru/journal*') ? $activeClass : $defaultClass }}">
                                <i class="fas fa-book-open w-5 h-5 mr-3"></i>
                                <span class="text-sm">Riwayat Jurnal</span>
                            </a>
                        </li>
